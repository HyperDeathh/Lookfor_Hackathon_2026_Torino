...
3.  **In `route.ts`**, we scan the entire conversation history *after* the execution.
...
```json
{
  ...
  "data": {
    ...
    "logs": [
      {
        "type": "tool_call", 
        "calls": [{ "name": "skio_get_subscription_status", "args": { "email": "jane@example.com" } }] 
      },
      {
        "type": "tool_output",
...
4.  **Update Prompt:** Tell the agent *when* to use it in its system prompt.
