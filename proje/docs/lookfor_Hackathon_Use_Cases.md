...
3. If it is still in transit, set the wait promise based on contact day.
...
5. If they reply after the promised date and it is still not delivered, escalate the ticket to let the human agent can process resend.
...
### Wrong or Missing Item in Parcel Process
...
5. If they do not want a reship, offer store credit first.
   - Offer store credit for the item value, plus a small bonus if your policy allows, for example 10%.
...
7. Close the loop.
   - If reship, escalate the ticket to support, so the support members can resend the order.
...
### Product Issue Process: No Effect
...
2. Ask for the reason for the refund request.
   ...
   **If the reason is shipping delay:**
   - If the customer contacts you on Monday or Tuesday, ask if they are okay waiting until Friday. If it is not delivered by then, offer a free replacement.
   - If the customer contacts you on Wednesday through Friday, ask if they are okay waiting until early next week. If it is not delivered by then, offer a free replacement.
   - If the customer refuses to wait, offer a free replacement immediately, then escalate the ticket so we can review. Tell customers this: "Hey, I'm looping Monica, who is our Head of CS, she'll take it from there."

   **If the reason is damaged or wrong item:**
...
**Sentiment:** Neutral  
**Notes:** Time-sensitive. Can be automated if <2 h.
...
2. Ask for the reason for the cancellation.
   ...
   **If the reason is a shipping delay:**
   - If the customer contacts you on Monday or Tuesday, ask if they are okay waiting until Friday. If it is not delivered by then, you can cancel it.
...
1. **Initial Response:**
   ...
   If it's okay with you, would you mind if I send you a feedback request so you can 
   share your thoughts on NATPAT and our response overall?

   It's totally fine if you don't have the time, but I thought I'd ask before sending 
   a feedback request email 

   Caz
   ```

2. **If they say YES and are happy to leave feedback:**
   ```
   Awwww, thank you! 
...
2. Ask for the reason.
   ...
   **If the reason is "too many on hand right now":**
   ...
   **If the reason is "didn't like the product quality":**
...
### Discount Code Problems Process

1. If a client wants to reclaim the code or is experiencing that the code is not valid, create a one-time discount code for that specific client. You can create a one-time 10% discount code with a 48-hour lifespan.

2. After you create the code, send it to the customer and let them know it is valid for 48 hours.
